    	...
    	<div class="the_types float-left">
    		{if is_array($page_types)}
    			{foreach $page_types as $type}
    				<article><input type="radio" value="{$type->option_value}" id="{$type->option_value}" name="page_type"> <label for="{$type->option_value}">{$type->option_value}</label></article>
    			{/foreach}
    		{else}
				...
				{if isset($extensions)}
					{if is_array($extensions)}
						{foreach $extensions as $extension}
							<option value="{$extension->option_name}" {if $post_options->extension eq $extension->option_name}selected{/if}>$extension->option_value</option>
						{/foreach}
					{/if}
				{else}
